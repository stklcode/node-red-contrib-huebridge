...
class Sensors {
    ...
    sensorsDispatcher(response, method, url, urlParts, obj) {
        ...
        */
        ...
        const api1 = /^\/api\/(\w+)\/sensors$/.exec(url);
        const api2 = /^\/api\/(\w+)\/sensors\/new$/.exec(url);
        const api3 = /^\/api\/(\w+)\/sensors\/(\w+)$/.exec(url);
        const api4 = /^\/api\/(\w+)\/sensors\/(\w+)\/config$/.exec(url);
        const api5 = /^\/api\/(\w+)\/sensors\/(\w+)\/state$/.exec(url);
