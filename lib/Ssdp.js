...
class SSDP {
    ...
    ssdpStart() {
        ...
        const uuid = 'uuid:2f402f80-da50-11e1-9b23-' + this.dsGetMAC().replace(/:/g, '');
        ...
        peer.on('notify', (headers, address) => {
            // this.debug('SSDP::start(peer-notify): address = ' + JSON.stringify(address));
            // this.debug('SSDP::start(peer-notify): headers = ' + JSON.stringify(headers));
        });

        peer.on(
            'search',
            (headers, address) => {
                // this.debug('SSDP::start(peer-search): address = ' + JSON.stringify(address));
                // this.debug('SSDP::start(peer-search): headers = ' + JSON.stringify(headers));
    ...
    _ssdpDiscovery(response) {
        ...
        let resp = `<root xmlns="urn:schemas-upnp-org:device-1-0">
    <specVersion>
        <major>1</major>
        <minor>0</minor>
    </specVersion>
