...
class Datastore {
     ...
     * Saves light and configuration data.
     */
    dsShutdown() {
    ...
    /**
     * Set the complete configuraiton.
     *
     * @param {*} obj New configuration object.
    ...
    dsSetEverything(obj) {
        ...
        if (!Object.prototype.hasOwnProperty.call(obj, 'groups')) {
            this.warn('Datastore::dsSetEverything(): groups object missing');
            return false;
        }
        if (!Object.prototype.hasOwnProperty.call(obj, 'schedules')) {
            this.warn('Datastore::dsSetEverything(): schedules object missing');
            return false;
        }
        if (!Object.prototype.hasOwnProperty.call(obj, 'scenes')) {
            this.warn('Datastore::dsSetEverything(): scenes object missing');
            return false;
        }
        if (!Object.prototype.hasOwnProperty.call(obj, 'sensors')) {
            this.warn('Datastore::dsSetEverything(): sensors object missing');
            return false;
        }
        if (!Object.prototype.hasOwnProperty.call(obj, 'rules')) {
            this.warn('Datastore::dsSetEverything(): rules object missing');
            return false;
        }
        if (!Object.prototype.hasOwnProperty.call(obj, 'resourcelinks')) {
     ...
     * Initialize lights.
     *
     * @param {boolean} clear Clear the light configuration?
     * @param {*}       obj   New configuration
     */
    ...
    /**
     * GEnerate new light ID.
     *
     * @param {string} clientid Client ID.
     * @return {boolean|number} Next light ID or FALSE on errors.
     */
    dsGetNewLightId(clientid) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.dsLights.nodelist.nodes, clientid)) {
            ...
            const newId = this.dsLights.nodelist.nextfree;
     ...
     * @return {number|boolean|*} Light ID or FALSE.
     */
    dsCreateLight(clientid, name, type, model) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.dsLights.nodelist.nodes, clientid)) {
            ...
            // Get the ID.
            for(let i in this.dsLights.clients) {
                if (this.dsLights.clients[i] === clientid) {
                    id = i;
                    break;
                }
            }
            if (id === null) {
                this.error('Datastore::dsCreateLight(): client exists but no ID found; clientid: ' + clientid);
                return false;
            }

            // Get current revision.
            l = this.dsLights.list[id];

            // Name or type changed?
            if (l.name !== name || l.type != type) {
                l.name = name;
                this.debug('Datastore::dsCreateLight(): client modified; clientid: ' + clientid);
            } else {
            ...
            // New client.
            id = this.dsGetNewLightId(clientid);
            l = {
                _typ: 0xFFFF,
        ...
        switch (type) {
            ...
            case '0x0200':  // Color Light - Color Gamut A
                l._typ = 0x0200;
                l.type = 'Color Light';
                l.modelid = 'LST001';
                l.capabilities = {
            ...
            case '0x0210':  // Extended Color Light - Color Gamut C
                l._typ = 0x0210;
    ...
    dsDeleteLight(id) {
        if (Object.prototype.hasOwnProperty.call(this.dsLights.list, id)) {
            var groupIds = this.dsGetAllGroupIDs();
            for (let g of groupIds) {
                this.dsRemoveLightFromGroup(g, id);
            }

            var sceneIds = this.dsGetAllSceneIDs();
            for (let s of sceneIds) {
                this.dsRemoveLightFromScene(s, id);
            }

            var scheduleIds = this.dsGetAllScheduleIDs();
            for (let s of scheduleIds) {
                this.dsRemoveLightFromSchedule(s, id);
            }

            var resourcelinkIds = this.dsGetAllResourcelinkIDs();
            for (let r of resourcelinkIds) {
                this.dsRemoveLightFromResourcelinks(r, id);
            }

            var ruleIds = this.dsGetAllRuleIDs();
            for (let r of ruleIds) {
                this.dsRemoveLightFromRule(r, id);
            }
    ...
    dsGetAllLightIDs() {
        return Object.keys(this.dsLights.list);
    }

    dsGetAllLightNodes() {
        const ids = this.dsGetAllLightIDs();
        const result = {};

        for (let id of ids) {
            result[id] = {
                clientid: this.dsLights.clients[id],
                type:     this.dsLights.list[id].type,
                _typ:     this.dsLights.list[id]._typ
            };
        }
    ...
    dsDeleteGroup(id) {
        if (Object.prototype.hasOwnProperty.call(this.dsGroups.list, id)) {
    ...
    dsDeleteScene(id) {
        if (Object.prototype.hasOwnProperty.call(this.dsScenes.list, id)) {
    ...
    dsInitSensors(clear, obj) {
        ...
        */
        if (!Object.prototype.hasOwnProperty.call(this.dsSensors.list, 1)) {
    ...
    dsGetNewSensorId(clientid) {
        if (typeof clientid === 'undefined') {
            const newId = this.dsSensors.nodelist.nextfree;
            ++this.dsSensors.nodelist.nextfree;

            return newId;
        } else if (typeof clientid === 'string') {
            if (Object.prototype.hasOwnProperty.call(this.dsSensors.nodelist.nodes, clientid)) {
                // client already exists
     ...
     * @return {number|boolean|string|*} Sensor ID or FALSE.
     */
    dsCreateSensor(typ, clientid, name) {
        if (typeof clientid !== 'string') {
            this.debug('Datastore::dsCreateSensor(): "clientid" must be a string');
            return false;
        }
        ...
        if (Object.prototype.hasOwnProperty.call(this.dsSensors.nodelist.nodes, clientid)) {
            ...
            // Get the ID.
            for(let i in this.dsSensors.clients) {
                if (this.dsSensors.clients[i] === clientid) {
                    id = i;
                    break;
                }
            }
            if (id === null) {
                this.error('Datastore::dsCreateSensor(): client exists but no ID found; clientid: ' + clientid);
                return false;
            }

            // Get current revision.
            s = this.dsSensors.list[id];

            // Name or type changed?
            if (s.name !== name || s.type != typ) {
                s.name = name;
                this.debug('Datastore::dsCreateSensor(): client modified; clientid: ' + clientid);
            } else {
                this.debug('Datastore::dsCreateSensor(): client already exists; clientid: ' + clientid);
                return s;
            }
        ...
        if (typeof typ !== 'undefined') {
            ...
            if (zll) {
                ...
                this.error('Datastore::dsCreateSensor(): undefined sensor type after initialization');
                return false;
            }
        }
    ...
    dsDeleteSensor(id) {
        if (Object.prototype.hasOwnProperty.call(this.dsSensors.list, id)) {
    ...
    dsGetAllSensorIDs() {
        return Object.keys(this.dsSensors.list);
    }
    ...
    dsDeleteRule(id) {
        if (Object.prototype.hasOwnProperty.call(this.dsRules.list, id)) {
    ...
    dsCreateResourcelinks(owner) {
        ...
        const id = this.dsGetNewResourcelinksId();
    ...
    dsDeleteResourcelinks(id) {
        if (Object.prototype.hasOwnProperty.call(this.dsResourcelinks.list, id)) {
    ...
    dsCreateSchedule() {
        ...
        const id = this.dsGetNewScheduleId();
    ...
    dsDeleteSchedule(id) {
        if (Object.prototype.hasOwnProperty.call(this.dsSchedules.list, id)) {
    ...
    dsGetAllScheduleIDs() {
        return Object.keys(this.dsSchedules.list);
    }

    dsRemoveLightFromSchedule(id, lightid) {
        this.debug('Datastore::dsRemoveLightFromSchedule() id = ' + id + ', lightid = ' + lightid);

        if (Object.prototype.hasOwnProperty.call(this.dsSchedules.list[id].command, 'address')) {
     ...
     *
     * @param {boolean} clear Clear capabilities?
     * @param {*}       obj   New capabilitiy object.
     */
    ...
    dsGetAllCapabilities() {
        ...
        try {
            var file = fs.readFileSync(__dirname + '/timezones.json');
            file = file.toString();
            tz = JSON.parse(file);
        } catch (err) {
    ...
    dsGetExternalAddress() {
        return this.dsExtAddress;
    }

    dsSetExternalAddress(extAddress) {
        this.dsExtAddress = extAddress;
    }

    dsGetExternalPort() {
        return this.dsExtPort;
    }

    dsSetExternalPort(extPort) {
        this.dsExtPort = extPort;
    }

    /**
     * Get the external URL.
     * If configured, external address and port will be used with fallback to default properties.
     *
     * @return {string}
     */
    dsGetExternalURL() {
        return (this.dsExtAddress || this.dsAddress) + ':' + (this.dsExtPort || this.dsHttpPort);
    }
    ...
    dsNewUsername(deviceType) {
        ...
        this.dsConfig.whitelist[username] = {
    ...
    dsDeleteUsername(username) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.dsConfig.whitelist, username)) {
